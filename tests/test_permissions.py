...
Server.__init__, so anything that built a Runner without a Server — `mha code`,
...
from mha.harnesses import registry
from mha.harnesses.lead.tools import CodeTool
from mha.llm.registry import ModelSpec, ProviderConfig, Registry
from mha.permissions import (
    AutoApproveBroker,
    ConsoleBroker,
    DenyBroker,
    GatedTool,
...
from mha.tools import BashTool, EditTool, ReadTool, ToolContext, WriteTool
...
def test_lead_dispatched_code_session_inherits_the_gate(tmp_path):
    ...
    import mha.harnesses.registry as reg_mod
...
def test_headless_broker_yes_flag_auto_approves():
    from mha.cli import _headless_broker
