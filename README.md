...
```
   ...
   bird ──────────▶│  lead — talks, researches, dispatches     │
                 └──────────────┬──────────────────┬────────┘
                                │                  │
                 ┌──────────────▼───────┐   ┌──────▼──────────────┐
                 │ arch — design on a   │──▶│ code — edit, test,  │
                 │ live browser canvas  │   │ verify the repo     │
                 └──────────────────────┘   └─────────────────────┘
...
- **A knowledge graph, not a grep loop.** The repo is extracted into a graph
  (`graphify`) — pure AST, no LLM, no API keys — and `kg_query` answers
...
- **Two layers, both always open.** A loose **sketch** layer (`variant`, `node`,
  ...
  is normal.
- **Disagreement is first-class.** `concern` records an objection — against the
  design, against a decision, or against *your* instruction — with a severity.
  Open blockers are shown at the finalize gate instead of silently blocking work.
...
- **Nothing is phase-locked.** Tools refuse only what is *broken* (an edge to a
  ...
  Post-approval structural edits record an amendment — the audit trail was the
  part worth keeping, not the lock.
...
- Permission brokers: interactive UI, console prompt, auto-approve (`--yes`), and
  deny-with-a-reason (the default when nobody can be asked — a gate no one can
  answer must never default to yes).
...
- `web_search` (DuckDuckGo, no API key) and `web_fetch` (HTML → markdown, cached
  15 min). Both excluded from eval control arms — a network-free eval shouldn't
  depend on them.

**Interfaces**
- Full-screen **TUI** (pi-tui, "Claude Native" palette) with keyboard-first
  permission cards showing real diffs — the default when installed.
...
`arch` and `code` — same engine, same toolbox, same permission gate — so they
gain the knowledge graph, session resume, model swapping and skills for free.
