...
Terminal UI for **mha**, implementing the "Claude Native" direction (part b) of
...
```bash
mha                               # the default interactive surface once
                                  # `npm install` has run in this directory
mha chat --plain                  # old line-based REPL instead
# or directly:
cd tui && npm start -- --repo /path/to/repo
npm start -- --demo               # scripted demo, no venv/ollama needed
```

The TUI spawns `mha serve` (JSON-lines over stdio) for the real harness:
model turns, tool calls, and the knowledge-graph context engine. Python is
resolved from `$MHA_PYTHON`, then the mha source venv, then the target repo's
venv, then `python3`.
...
- Type a message, `⏎` to send, `⇧⏎` newline (`⌥⏎` in terminals that don't
  report shift)
- `/` opens the command drawer (mha's real REPL commands, executed by the
...
- Permission cards appear for `edit`/`write` tool calls with a real diff:
  `Y`/`⏎` approve, `N`/`esc` deny. Bash stays ungated because mha allowlists
