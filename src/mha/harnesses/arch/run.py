...
as `mha arch`, but returning the finalized ArchSession to the caller instead of
...
def run_arch_interactive(
    *,
    ...
    cli._arch_main's bring-up; kept separate so `mha arch`'s resume path stays
    untouched."""
    import sys
    import time
    import webbrowser
    ...
    with SessionRecorder(run_dir) as recorder:
        ...
        if on_status is not None:
            on_status(banner)
        # also to stderr: the TUI surfaces `mha serve` stderr as a notice, so the
