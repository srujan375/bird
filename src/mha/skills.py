...
Discovery order (first wins on name collision):
  1. project skills  — `.mha/skills/*.md` (repo-local, version-controlled)
  2. user skills    — `~/.mha/skills/*.md` (personal, cross-project)
  3. built-in skills — packaged inside mha (`src/mha/skills/builtin/*.md`)

Built-in skills ship with mha so features like `skill-creator` are available
...
def load_skills(repo_root: Path) -> list[Skill]:
    ...
    """
    project = _scan_dir(repo_root / ".mha" / "skills", "project")
    user = _scan_dir(Path.home() / ".mha" / "skills", "user")
