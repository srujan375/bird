...
explicit via `MHA_KG_BACKEND` (any name in `graphify.llm.BACKENDS`, or
...
per-branch under `.mha/kg/<branch-slug>/graphify-out/` so switching
...
class KG:
    def __init__(
        ...
        self.semantic_backend = semantic_backend or os.environ.get("MHA_KG_BACKEND") or None
        self.semantic_model = semantic_model or os.environ.get("MHA_KG_MODEL") or None
        self.out_dir = (
            Path(store_dir)
            if store_dir
            else self.repo_root / ".mha" / "kg" / branch_slug(self.repo_root) / "graphify-out"
        )
    ...
    def ensure_background(self) -> subprocess.Popen | None:
        ...
        return subprocess.Popen(
            [sys.executable, "-m", "mha.context.kg", str(self.repo_root), str(self.out_dir)],
            stdout=log,
