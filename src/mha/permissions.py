...
existed — so `mha code`, the plain REPL, and every sub-harness the lead
...
A broker is anything with `.request(payload) -> (approved, feedback)`:
  ...
  AutoApproveBroker approves everything — `mha code --yes`, unattended runs.
...
class AutoApproveBroker:
    """Approves everything. `mha code --yes` and other unattended runs."""
