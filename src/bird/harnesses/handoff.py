...
`.bird/sessions/<run_id>/bundle/architecture.md`; a code session seeds itself
...
decision. Both `bird code --from-arch` and the lead's `code` tool use it.
"""
...
def sessions_dir(repo_root: Path) -> Path:
    return repo_root / ".bird" / "sessions"
