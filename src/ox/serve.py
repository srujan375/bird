"""ox serve — the session pump plus its transports.
...
HttpTransport (ox.http_transport) carries the same events over SSE + POSTs
...
Permission gating itself lives in ox.permissions and attaches at runner
...
why: gating in this file left `ox code`, the plain REPL, and every
...
from .permissions import (  # re-exported: importers still say ox.serve.GatedTool
    DIFF_CONTEXT_LINES,
    MAX_DIFF_LINES,
    GatedTool,
    PermissionBroker,
...
class Server:
    ...
    def _command(self, line: str) -> bool | None:
        ...
        if line.strip() in ("/reload", "/reload-skills"):
            ...
            # owns the process and respawns `ox serve` fresh from disk. We
