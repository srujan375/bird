...
reads to the user as a path bug in ox rather than as macOS cleaning up.

The fix is to copy at the seam where the *user* hands ox the path, not at the
...
the user picked the file, and ox is only making sure it still exists by the
