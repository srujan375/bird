...
as `ox arch`, but returning the finalized ArchSession to the caller instead of
...
def run_arch_interactive(
    *,
    ...
    cli._arch_main's bring-up; kept separate so `ox arch`'s resume path stays
    untouched."""
    import sys
    import time
    import webbrowser
    ...
    with SessionRecorder(run_dir) as recorder:
        ...
        transport = HttpTransport(
            ...
            # no linger here, unlike `ox arch`: the lead is blocked on this
        ...
        if on_status is not None:
            on_status(banner)
        # also to stderr: the TUI surfaces `ox serve` stderr as a notice, so the
