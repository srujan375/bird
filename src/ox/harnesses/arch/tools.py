...
from ...tools.files import ReadImageTool, ReadTool
...
class ArchDoneTool(Tool):
    ...
    def _finalize_gate(self, session: ArchSession, summary: str, kg: Any = None) -> ToolResult:
        ...
        if seeded:
            tail += f" {seeded}."
        return ToolResult(
            output=f"Architecture finalized. Handoff bundle: {paths}.{tail} Next step: ox code.",
...
class ImportStateTool(Tool):
    ...
    def run(self, args: dict[str, Any], ctx: ToolContext) -> ToolResult:
        ...
        if kg is None or not kg.is_ready():
            raise ToolError(
                "the knowledge graph is not ready; run `ox kg build` first, or "
...
def arch_harness_tools(with_kg: bool = True, with_web: bool = True) -> list[Tool]:
    ...
    tools: list[Tool] = [ReadTool(), ReadImageTool()]
    if with_kg:
