"""Session recording: append-only events.jsonl under .ox/sessions/<run-id>/."""
...
def load_messages(run_dir: Path) -> list[dict[str, Any]] | None:
    ...
    sessions recorded by older ox versions that didn't separate the
...
def read_session_meta(run_dir: Path) -> dict[str, Any]:
    ...
    'name' (a human label, set by /rename or by `ox` on creation)."""
